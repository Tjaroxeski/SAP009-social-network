...
const login$1 = (email, senha) => signInWithEmailAndPassword(auth, email, senha);
const logingoogle = () => {
  const authprovider = new GoogleAuthProvider();
  return signInWithPopup(auth, authprovider);
};
const criarUsuario = (displayName, email, senha) => createUserWithEmailAndPassword(auth, email, senha).then((userCredential) => {
  const usuario = userCredential.user;
  return updateProfile(usuario, { displayName });
});
const logomobile = "/assets/logomobile.67209e71.png";
const logodesktop = "/assets/logodesktop.9992bfe9.png";
const logogoogle = "/assets/google.c58a297d.png";
const login = () => {
...
const cadastro = () => {
  ...
  function errorMessage(error) {
    if (error.code === "auth/email-already-exists") {
      return "Email j\xE1 cadastrado.";
    }
    if (error.code === "auth/invalid-password") {
      return "A senha precisa ter no m\xEDnimo 6 caracteres.";
    }
  ...
  const cadastro2 = container.querySelector("#confirmar");
  cadastro2.addEventListener("click", (event) => {
    ...
    if (nomeCompleto.value === "" || apelido.value === "" || email.value === "" || senha.value === "") {
      ...
      criarUsuario(nomeCompleto.value, email.value, senha.value).then(() => {
...
class tl extends Gh {
  _toFieldTransform(t) {
    if (2 !== t.ua)
      throw 1 === t.ua ? t._a(`${this._methodName}() can only appear at the top level of your update data`) : t._a(`${this._methodName}() cannot be used with set() unless you pass {merge:true}`);
...
function ol(t, e, n, s) {
  const i = t.ga(1, e, n);
  fl("Data must be an object, but it was:", i, s);
  const r2 = [], o = un.empty();
  ge(s, (t2, s2) => {
    const u2 = wl(e, t2, n);
  ...
  const u = new ve(r2);
  return new Wh(o, u, i.fieldTransforms);
}
function ul(t, e, n, s, i, r2) {
  const o = t.ga(1, e, n), u = [dl(e, s, n)], c = [i];
  if (r2.length % 2 != 0)
  ...
  for (let t2 = 0; t2 < r2.length; t2 += 2)
    u.push(dl(e, r2[t2])), c.push(r2[t2 + 1]);
  const a = [], h = un.empty();
  ...
  const l2 = new ve(a);
  return new Wh(h, l2, o.fieldTransforms);
}
  ...
  _apply(t) {
    const e = function(t2, e2, n) {
      ...
      return function(t3, e3) {
        if (null === jn(t3)) {
          const n2 = zn(t3);
          null !== n2 && Ql(t3, n2, e3.field);
        }
      }(t2, s), s;
    }(t._query, this._field, this._direction);
    return new fh(t.firestore, t.converter, function(t2, e2) {
...
function Cl(t, e = "asc") {
  const n = e, s = Il("orderBy", t);
  return Dl._create(s, n);
}
function Ul(t, e, n) {
...
function mf(t, e, n, ...s) {
  t = oh(t, lh);
  const i = oh(t.firestore, Ah), r2 = Xh(i);
  let o;
  o = "string" == typeof (e = getModularInstance(e)) || e instanceof Uh ? ul(r2, "updateDoc", t._key, e, n, s) : ol(r2, "updateDoc", t._key, e);
  return Tf(i, [o.toMutation(t._key, Fs.exists(true))]);
}
function gf(t) {
...
const db = vh(app);
const newPost = (postagem, dataPostagem, username, id2) => yf(_h(db, "post"), {
  data: dataPostagem,
  post: postagem,
  idUser: id2,
  username
...
const accessPost = async () => {
  const messages = [];
  const queryOrder = Rl(_h(db, "post"), Cl("data", "desc"));
  const querySnapshot = await ff(queryOrder);
...
const editPost = (postId, textArea) => mf(mh(db, "post", postId), {
  post: textArea
});
const deletePost = async (postId) => gf(mh(db, "post", postId));
const airfryerfeed = "/assets/airfryerfeed.36638da2.png";
const feed = () => {
  const container = document.createElement("div");
  const template = `
    ...
    arrayPosts.forEach((post) => {
      if (post.idUser === auth.currentUser.uid) {
        ...
        btnEditar.addEventListener("click", (e) => {
          e.preventDefault();
          if (window.confirm("Tem certeza de que deseja editar a publica\xE7\xE3o?")) {
